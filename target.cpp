...
@ exponential (int joystickVal, float driveExp, int joyDead, int motorMin): //e
    @ joySign, joyMax, joyLive;
    joyMax = 128 - joyDead;
    joyLive = abs(joystickVal) - joyDead;
    if  joystickVal > 0:
        joySign = 1;
        ...
        joySign = 0;
    return joySign * (motorMin + ((127 - motorMin) *
    (pow(joyLive, driveExp) / pow(joyMax, driveExp))));

void Chassis::OP_Chassis():
